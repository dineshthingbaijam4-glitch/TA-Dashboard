      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
        <div style={S.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            ...
            <span style={{ fontSize: "10px", color: "#e85d3a", fontWeight: "600" }}>{data.openPositions.length} total</span>
          </div>
          {(() => {
            const centreMap = {};
            data.openPositions.forEach(p => {
              const c = p.centre || "Unknown";
              if (!centreMap[c]) centreMap[c] = 0;
              centreMap[c]++;
            });
            const centres = Object.entries(centreMap).sort((a, b) => b[1] - a[1]);
            const max = centres[0]?.[1] || 1;
            const colors = ["#e85d3a","#8b5cf6","#3b82f6","#10b981","#f59e0b","#f97316","#6366f1","#ec4899"];
            return centres.map(([centre, count], i) => (
              <div key={centre} style={{ marginBottom: "12px", cursor: "pointer" }}
                onClick={() => onOpenModal(`${centre} — Open Positions`, data.openPositions.filter(p => p.centre === centre))}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "5px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: colors[i % colors.length], flexShrink: 0 }} />
                    <span style={{ fontSize: "12px", color: "#ccc", fontWeight: "500" }}>{centre}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <span style={{ fontSize: "12px", fontWeight: "700", color: colors[i % colors.length] }}>{count}</span>
                    <span style={{ fontSize: "9px", color: "#444" }}>view →</span>
                  </div>
                </div>
                <div style={{ height: "5px", background: "rgba(255,255,255,0.06)", borderRadius: "3px", overflow: "hidden" }}>
                  <div style={{ height: "100%", borderRadius: "3px", width: `${(count / max) * 100}%`, background: colors[i % colors.length], transition: "width 0.8s ease" }} />
                </div>
              </div>
            ));
          })()}
