...
function DashboardTab({ data, isLive, onOpenModal, offerEntries }) {
  ...
  const yetToJoin = offered; // "Offered" = offer given, not yet joined/confirmed
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: "14px", marginBottom: "24px" }}>
        ...
        <StatCard
          ...
          sub={`${joined.length} joined · ${notJoined.length} not joining · ${offered.length} pending`}
          color="#06b6d4" icon="🤝"
          clickable={allOffered.length > 0}
          onClick={() => allOffered.length > 0 && onOpenModal("Offer → Joining", allOffered.map((e, i) => ({
            id: i, role: e.role || e.designation || "", centre: e.centre || "", hr: "", step: 6,
            daysOpen: 0, status: "offer", priority: "medium",
            offerCandidate: e.name || "", expectedClosure: e.doj || "",
            joiningStatus: e.joiningStatus || "", _action: "",
          })))}
        />
        <StatCard
          label="Yet to Join" value={yetToJoin.length}
          sub={useOfferSheet ? "status = Offered in joining tracker" : "offer & DOJ set, status pending"}
          color="#8b5cf6" icon="🕐"
          clickable={yetToJoin.length > 0}
          onClick={() => yetToJoin.length > 0 && onOpenModal("Yet to Join", yetToJoin.map((e, i) => ({
            id: i, role: e.role || "", centre: e.centre || "", hr: "", step: 6,
            daysOpen: 0, status: "offer", priority: "medium",
            offerCandidate: e.name || "", expectedClosure: e.doj || "",
            joiningStatus: e.joiningStatus || "", _action: "",
          })))}
    ...
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        <div style={{ padding: "24px 28px" }}>
          {activeTab === "dashboard"  && <DashboardTab  data={data} isLive={isLive} onOpenModal={openModal} offerEntries={offerEntries} />}
