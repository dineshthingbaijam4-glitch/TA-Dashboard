...
function PositionModal({ title, positions, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: "#ffffff", border: "1px solid #e8e2da", borderRadius: "16px", width: "90%", maxWidth: "960px", maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: "18px 24px", borderBottom: "1px solid #e8e2da", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
          ...
          <button onClick={onClose} style={{ background: "#f0ebe4", border: "1px solid #e8e2da", borderRadius: "8px", color: "#8a8279", fontSize: "13px", padding: "6px 12px", cursor: "pointer" }}>✕ Close</button>
        </div>
        <div style={{ overflow: "auto", flex: 1 }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead style={{ position: "sticky", top: 0, background: "#faf8f5", zIndex: 1 }}>
              <tr>
                {["Role", "Centre", "HR Owner", "Step", "Days Open", "Expected Closure", "Status", "Offer Candidate", "DOJ"].map(h => (
                  <th key={h} style={{ padding: "10px 14px", textAlign: "left", fontSize: "9px", color: "#8a8279", fontWeight: "600", letterSpacing: "0.08em", textTransform: "uppercase", borderBottom: "1px solid #e8e2da", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {positions.length === 0 && (
                <tr><td colSpan={9} style={{ padding: "40px", textAlign: "center", color: "#8a8279" }}>No positions found</td></tr>
              )}
              {positions.map((pos, i) => {
                ...
                return (
                  <tr key={i} style={{ borderBottom: "1px solid #f0ebe4", background: isOverdue ? "#fdf2ef" : "transparent" }}
                    onMouseEnter={e => e.currentTarget.style.background = "#faf8f5"}
                    onMouseLeave={e => e.currentTarget.style.background = isOverdue ? "#fdf2ef" : "transparent"}>
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", gap: "2px", marginBottom: "3px" }}>
                        {[1,2,3,4,5,6,7].map(s => <div key={s} style={{ width: "14px", height: "3px", borderRadius: "2px", background: s <= pos.step ? "#5a9a8f" : "#e8e2da" }} />)}
                      </div>
                      <div style={{ fontSize: "9px", color: "#8a8279" }}>Step {pos.step}</div>
                    </td>
                    <td style={{ padding: "12px 14px" }}>
                      <span style={{ fontSize: "12px", fontWeight: "700", color: pos.daysOpen > 14 ? "#c4786a" : pos.daysOpen > 7 ? "#b8956a" : "#5a9a8f" }}>{pos.daysOpen}d</span>
                    </td>
                    <td style={{ padding: "12px 14px", fontSize: "11px", color: isOverdue ? "#c4786a" : "#8a8279", fontWeight: isOverdue ? "600" : "400" }}>
                      {pos.expectedClosure || "—"}
                    ...
                    <td style={{ padding: "12px 14px", fontSize: "11px", color: pos.doj ? "#5a9a8f" : "#8a8279", fontWeight: pos.doj ? "600" : "400" }}>{pos.doj || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
...
function Toast({ message, type = "success", onClose }) {
  ...
  return (
    <div style={{ position: "fixed", bottom: "24px", right: "24px", zIndex: 1000, background: type === "error" ? "#c4786a" : "#5a9a8f", color: "white", padding: "12px 20px", borderRadius: "10px", fontSize: "13px", fontWeight: "600", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", maxWidth: "320px", lineHeight: "1.4" }}>
      {type === "error" ? "❌ " : "✅ "}{message}
    </div>
  );
}
...
function SettingsTab({ config, onSave, onTest }) {
  ...
  const Field = ({ label, fkey, testKey, placeholder, hint }) => (
    <div style={{ marginBottom: "18px" }}>
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        {testKey && (
          <button onClick={() => handleTest(testKey)} style={{ ...S.btn, background: testing[testKey] === "ok" ? "rgba(90,154,143,0.1)" : "#faf8f5", border: "1px solid #d4cdc3", color: testing[testKey] === "ok" ? "#5a9a8f" : "#8a8279", padding: "10px 14px", whiteSpace: "nowrap" }}>
            {testIcon(testKey)}
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ maxWidth: "680px" }}>
      ...
      <p style={{ margin: "0 0 24px", fontSize: "12px", color: "#8a8279", lineHeight: "1.6" }}>Paste your Google Sheet links below. Make sure each sheet is set to <strong style={{ color: "#5a9a8f" }}>"Anyone with the link — Viewer"</strong> in sharing settings.</p>
      ...
      <div style={{ ...S.card, marginBottom: "16px" }}>
        <div style={{ fontSize: "13px", fontWeight: "700", color: "#3a3a3a", fontFamily: "'Syne',sans-serif", marginBottom: "18px", paddingBottom: "12px", borderBottom: "1px solid #e8e2da" }}>📊 Google Sheets Trackers</div>
      ...
      <div style={{ ...S.card, marginBottom: "16px" }}>
        <div style={{ fontSize: "13px", fontWeight: "700", color: "#3a3a3a", fontFamily: "'Syne',sans-serif", marginBottom: "18px", paddingBottom: "12px", borderBottom: "1px solid #e8e2da" }}>🔔 Alert Settings</div>
        ...
        <Field label="Centre HR Emails (comma separated)" fkey="centreHREmails" placeholder="nisarga.d@locobear.com, bosky.p@locobear.com" hint="Reminder emails will be sent to these addresses automatically" />
        <div>
...
function CentreBreakdown({ positions, onOpenModal }) {
  ...
  return (
    <div style={S.card}>
      ...
      {sorted.length === 0 && <div style={{ fontSize: "12px", color: "#8a8279", textAlign: "center", padding: "20px 0" }}>No active positions</div>}
      {sorted.map(([centre, count], i) => {
        const pct = total > 0 ? Math.round((count / total) * 100) : 0;
        const colour = CENTRE_COLOURS[i % CENTRE_COLOURS.length];
        return (
          <div key={centre} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: i < sorted.length - 1 ? "1px solid #f0ebe4" : "none", cursor: "pointer", borderRadius: "4px" }}
            ...
            <div style={{ fontSize: "11px", color: "#4a4a4a", minWidth: "90px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{centre}</div>
            <div style={{ flex: 1, height: "4px", background: "#f0ebe4", borderRadius: "2px", overflow: "hidden" }}>
...
function RoleBreakdown({ positions, onOpenModal }) {
  ...
  const total = active.length;
  const roleMap = {};
  active.forEach(p => { const key = (p.role || "Unknown").trim(); roleMap[key] = (roleMap[key] || 0) + 1; });
  ...
  return (
    <div style={S.card}>
      ...
      {sorted.map(([role, count], i) => {
        ...
        const colour = CENTRE_COLOURS[i % CENTRE_COLOURS.length];
        return (
          <div key={role} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: i < sorted.length - 1 ? "1px solid #f0ebe4" : "none", cursor: "pointer", borderRadius: "4px" }}
            onClick={() => onOpenModal(`Role: ${role}`, active.filter(p => p.role === role))}
            onMouseEnter={e => e.currentTarget.style.background = "#faf8f5"}
            onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            <div style={{ width: "7px", height: "7px", borderRadius: "50%", background: colour, flexShrink: 0 }} />
            <div style={{ fontSize: "11px", color: "#4a4a4a", minWidth: "120px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flex: 1 }}>{role}</div>
            <div style={{ flex: 2, height: "4px", background: "#f0ebe4", borderRadius: "2px", overflow: "hidden" }}>
              <div style={{ height: "100%", borderRadius: "2px", width: `${pct}%`, background: colour, transition: "width 0.4s ease" }} />
            </div>
            <span style={{ fontSize: "12px", fontWeight: "700", color: "#3a3a3a", minWidth: "18px", textAlign: "right" }}>{count}</span>
            <span style={{ fontSize: "10px", color: "#8a8279", minWidth: "32px", textAlign: "right" }}>{pct}%</span>
...
function DashboardTab({ data, isLive, onOpenModal }) {
  ...
  const StatCard = ({ label, value, sub, color, icon, onClick, clickable }) => (
    <div onClick={onClick} style={{ ...S.card, borderTop: `3px solid ${color}`, cursor: clickable ? "pointer" : "default", transition: "transform 0.15s, box-shadow 0.15s" }}
      ...
      <div style={{ fontSize: "10px", color: "#aaa09a", marginTop: "2px" }}>{sub}</div>
    ...
    <div>
      {isLive ? (
        <div style={{ display: "inline-flex", alignItems: "center", gap: "7px", padding: "6px 14px", borderRadius: "20px", marginBottom: "20px", background: "#e8f5f1", border: "1px solid #a8d5c5" }}>
          <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "#5a9a8f", animation: "pulse 2s infinite" }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: "14px", marginBottom: "24px" }}>
        <StatCard
          ...
          sub="across all centres" color="#6a6ab0" icon="📋" clickable={true}
        ...
        <StatCard
          ...
          color="#6a9ab0" icon="🤝" clickable={offered.length > 0}
          onClick={() => offered.length > 0 && onOpenModal("Offer → Joining", offered)}
        />
        <StatCard
          label="Yet to Join" value={pending.length}
          sub="offer & DOJ set, status pending" color="#8a6ab0" icon="🕐" clickable={pending.length > 0}
          onClick={() => pending.length > 0 && onOpenModal("Yet to Join", pending)}
        />
        <StatCard
          label="Overdue Positions" value={overduePositions.length}
          sub="past expected closure date" color="#c4786a" icon="⚠️" clickable={overduePositions.length > 0}
          onClick={() => overduePositions.length > 0 && onOpenModal("Overdue Positions", overduePositions)}
        />
        <StatCard
          label="Avg Compliance" value={`${Math.round(data.compliance.reduce((a, c) => a + c.percent, 0) / (data.compliance.length || 1))}%`}
          sub="across all Centre HRs" color="#5a9a8f" icon="✅" clickable={false}
        />
      </div>
          ...
          {STEP_LABELS.slice(1).map((step, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, textAlign: "center", cursor: count > 0 ? "pointer" : "default" }}
                ...
                <div style={{ background: count > 0 ? "#e8f5f1" : "#faf8f5", border: count > 0 ? "1px solid #a8d5c5" : "1px solid #e8e2da", borderRadius: "10px", padding: "12px 4px", transition: "all 0.15s" }}
                  onMouseEnter={e => { if (count > 0) e.currentTarget.style.background = "#d8ede8"; }}
                  onMouseLeave={e => { if (count > 0) e.currentTarget.style.background = "#e8f5f1"; }}>
    ...
    <div>
      <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
        {["all", "new", "screening", "interview", "final", "offer"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{ padding: "6px 14px", borderRadius: "20px", border: "1px solid #e8e2da", cursor: "pointer", fontSize: "11px", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.05em", background: filter === f ? "#5a9a8f" : "#ffffff", color: filter === f ? "white" : "#8a8279", transition: "all 0.15s" }}>{f}</button>
        ))}
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            ...
            {filtered.map(pos => {
              const sc = STATUS_CONFIG[pos.status] || STATUS_CONFIG.new;
              return (
                <tr key={pos.id} style={{ borderBottom: "1px solid #f0ebe4" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#faf8f5"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  ...
                  <td style={{ padding: "13px 14px", minWidth: "140px" }}>
                    <div style={{ display: "flex", gap: "3px", marginBottom: "4px" }}>
                      {[1,2,3,4,5,6,7].map(s => <div key={s} style={{ flex: 1, height: "4px", borderRadius: "2px", background: s < pos.step ? "#5a9a8f" : s === pos.step ? "#b8956a" : "#f0ebe4" }} />)}
                    </div>
                    <div style={{ fontSize: "9px", color: "#8a8279" }}>Step {pos.step} — {STEP_LABELS[pos.step]}</div>
                  </td>
                  <td style={{ padding: "13px 14px" }}><span style={{ fontSize: "13px", fontWeight: "700", fontFamily: "'Syne',sans-serif", color: pos.daysOpen > 14 ? "#c4786a" : pos.daysOpen > 7 ? "#b8956a" : "#5a9a8f" }}>{pos.daysOpen}d</span></td>
                  ...
                  <td style={{ padding: "13px 14px" }}><span style={{ fontSize: "9px", fontWeight: "700", textTransform: "uppercase", color: pos.priority === "high" ? "#c4786a" : pos.priority === "medium" ? "#b8956a" : "#8a8279" }}>{pos.priority}</span></td>
                </tr>
              );
            })}
          </tbody>
...
function ComplianceTab({ data, onRemind }) {
  return (
    <div style={{ display: "grid", gap: "14px" }}>
      {data.compliance.length === 0 && <div style={{ ...S.card, textAlign: "center", padding: "60px", color: "#8a8279" }}>No compliance data available</div>}
...
function AlertsTab({ data, onRemind }) {
  return (
    <div style={{ display: "grid", gap: "10px" }}>
      {data.alerts.length === 0 && <div style={{ ...S.card, textAlign: "center", padding: "60px", color: "#8a8279" }}>No alerts</div>}
...
export default function App() {
  ...
  const loadLiveData = useCallback(async (cfg) => {
    ...
    try {
      ...
      if (positions.length > 0) {
        ...
        showToast("Sheet connected but no data found. Check your column names.", "error");
      }
    } catch (e) {
  ...
  return (
    <div style={S.app}>
      <style>{`* { box-sizing:border-box; } ::-webkit-scrollbar{width:4px} ::-webkit-scrollbar-track{background:#f5f0eb} ::-webkit-scrollbar-thumb{background:#d4cdc3;border-radius:2px} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}} input::placeholder{color:#a0988e} input:focus{border-color:#5a9a8f!important} @import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Sans:wght@300;400;500;600&display=swap');`}</style>
      ...
      <div style={S.sidebar}>
        ...
        <div style={{ padding: "10px 16px", borderTop: "1px solid #e8e2da" }}>
        ...
        <div style={{ padding: "12px 16px", borderTop: "1px solid #e8e2da" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "9px" }}>
            ...
            <div>
              <div style={{ fontSize: "11px", fontWeight: "600", color: "#3a3a3a" }}>Dinesh T.</div>
      ...
      <div style={S.main}>
        <div style={{ padding: "18px 28px", borderBottom: "1px solid #e8e2da", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10, background: "rgba(250,248,245,0.97)", backdropFilter: "blur(12px)" }}>
          <div>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {config.taTrackerUrl && <button onClick={() => loadLiveData(config)} style={{ padding: "7px 14px", borderRadius: "8px", fontSize: "11px", background: "#e8f5f1", border: "1px solid #a8d5c5", color: "#5a9a8f", cursor: "pointer", fontWeight: "500" }}>🔄 Refresh</button>}
            <button onClick={() => handleRemind({ name: "all Centre HRs" })} style={{ padding: "7px 14px", borderRadius: "8px", fontSize: "11px", background: "#faf8f5", border: "1px solid #e8e2da", color: "#8a8279", cursor: "pointer" }}>📧 Send All Reminders</button>
