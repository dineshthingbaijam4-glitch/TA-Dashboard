...
const MOCK_DATA = {
  ...
  tatCount: 2,
  openPositions: [
    { id: 1, role: "Marketing Manager", centre: "Bangalore", step: 4, daysOpen: 12, hr: "Nisarga D.", status: "interview", priority: "high", joiningStatus: "", doj: "", _receiveDate: "2026-06-01", _closeDate: "", _parsedReceive: new Date("2026-06-01"), _parsedClose: null },
    { id: 2, role: "Games Experience Executive", centre: "Loco Lane", step: 3, daysOpen: 8, hr: "Nisarga D.", status: "screening", priority: "medium", joiningStatus: "", doj: "", _receiveDate: "2026-06-05", _closeDate: "", _parsedReceive: new Date("2026-06-05"), _parsedClose: null },
    { id: 3, role: "Performance Marketing", centre: "Bangalore", step: 5, daysOpen: 18, hr: "Bosky P.", status: "final", priority: "high", joiningStatus: "", doj: "", _receiveDate: "2026-05-25", _closeDate: "2026-06-10", _parsedReceive: new Date("2026-05-25"), _parsedClose: new Date("2026-06-10") },
    { id: 4, role: "Senior TA Executive", centre: "Dubai", step: 2, daysOpen: 5, hr: "Wilfred W.", status: "tracker", priority: "medium", joiningStatus: "", doj: "", _receiveDate: "2026-06-08", _closeDate: "", _parsedReceive: new Date("2026-06-08"), _parsedClose: null },
    { id: 5, role: "Event Coordinator", centre: "Raipur", step: 1, daysOpen: 3, hr: "HR Raipur", status: "new", priority: "low", joiningStatus: "", doj: "", _receiveDate: "2026-06-10", _closeDate: "", _parsedReceive: new Date("2026-06-10"), _parsedClose: null },
    { id: 6, role: "Event Coordinator", centre: "Raipur", step: 1, daysOpen: 3, hr: "HR Raipur", status: "new", priority: "low", joiningStatus: "", doj: "", _receiveDate: "2026-06-10", _closeDate: "", _parsedReceive: new Date("2026-06-10"), _parsedClose: null },
    { id: 7, role: "Brand Executive", centre: "Rebounce", step: 6, daysOpen: 22, hr: "Pallavi V.", status: "offer", priority: "high", joiningStatus: "", doj: "", _receiveDate: "2026-05-20", _closeDate: "2026-06-05", _parsedReceive: new Date("2026-05-20"), _parsedClose: new Date("2026-06-05") },
    { id: 8, role: "Operations Lead", centre: "Brigade", step: 3, daysOpen: 10, hr: "Bosky P.", status: "screening", priority: "medium", joiningStatus: "", doj: "", _receiveDate: "2026-06-03", _closeDate: "", _parsedReceive: new Date("2026-06-03"), _parsedClose: null },
  ],
};
...
function parseFlexibleDate(raw) {
  ...
  const nm = s.match(/^(\d{1,2})[-\s]([A-Za-z]{3,})[-\s](\d{4})$/);
  if (nm) { const mo = MONTHS[nm[2].slice(0,3).toLowerCase()]; if (mo !== undefined) return new Date(parseInt(nm[3]), mo, parseInt(nm[1])); }
  const nmr = s.match(/^([A-Za-z]{3,})[-\s](\d{1,2})[-\s](\d{4})$/);
  if (nmr) { const mo = MONTHS[nmr[1].slice(0,3).toLowerCase()]; if (mo !== undefined) return new Date(parseInt(nmr[3]), mo, parseInt(nmr[2])); }
  ...
  if (c > 1900) {
    ...
    return sep === "-" ? new Date(c, a - 1, b) : new Date(c, b - 1, a);
  }
...
function inferStepFromRow(row) {
  ...
  const posClose = row["Position close date / Offer Letter release date"] || "";
...
function calcDaysOpen(row) {
  const d = parseFlexibleDate(row["Position receive date"] || "");
  if (!d || isNaN(d)) return 0;
  return Math.max(0, Math.floor((new Date() - d) / 86400000));
}

// Returns { positions, avgTat, tatCount } — TAT computed across ALL rows before action filter
function parseSheetToPositions(rows) {
  const allMapped = rows.map((row, i) => {
    ...
    const parsedReceive = parseFlexibleDate(rawReceive);
    const parsedClose   = parseFlexibleDate(rawClose);
    return {
      id: i + 1,
      ...
      _closeDate: rawClose,
      _parsedReceive: parsedReceive,
      _parsedClose: parsedClose,
    };
  }).filter(p => p.role && p.role !== "");

  // TAT: all rows where BOTH dates parse successfully and close >= receive
  const tatRows = allMapped.filter(p =>
    p._parsedReceive && p._parsedClose &&
    !isNaN(p._parsedReceive) && !isNaN(p._parsedClose) &&
    p._parsedClose >= p._parsedReceive
  );

  const avgTat = tatRows.length > 0
    ? Math.round(tatRows.reduce((sum, p) =>
        sum + Math.floor((p._parsedClose - p._parsedReceive) / 86400000), 0
      ) / tatRows.length)
    : null;

  const tatCount = tatRows.length;

  // Active positions for display: only "in progress"
  const activePositions = allMapped.filter(p => {
    const action = (p._action || "").toLowerCase().trim();
    return action === "in progress" || action === "inprogress";
  });

  // Return as plain object — no custom array properties
  return { positions: activePositions, avgTat, tatCount };
}
...
function PositionModal({ title, positions, onClose, T, showDoj, showOffer }) {
  ...
  const colCount = showOffer ? 6 : showDoj ? 4 : 7;
            ...
            <tbody>
              ...
              {displayPositions.map((pos, i) => {
                ...
                if (showDoj) {
                  const sc = statusColor(pos.joiningStatus);
                  return (
                    <tr key={i} style={{ borderBottom:`1px solid ${T.tableBdr}` }}
                      onMouseEnter={e => e.currentTarget.style.background = T.rowHover}
...
function DashboardTab({ data, isLive, onOpenModal, onOpenDojModal, onOpenOfferModal, offerEntries, T }) {
  ...
  const recentTop5 = [...data.openPositions]
    .filter(p => p._parsedReceive && !isNaN(p._parsedReceive))
    .sort((a, b) => b._parsedReceive - a._parsedReceive)
    .slice(0, 5);
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"14px", marginBottom:"24px" }}>
        ...
        <StatCard
          ...
          sub={data.tatCount > 0 ? `avg of ${data.tatCount} closed/offered roles` : "no closed roles with both dates yet"}
          color="#10b981" icon="⏱️" clickable={false}
        />
        <StatCard
          label="Recently Added Roles"
          value={recentTop5.length}
          sub="top 5 most recently received"
          color="#f59e0b" icon="🆕" clickable={recentTop5.length>0}
          onClick={() => recentTop5.length>0 && onOpenModal("Recently Added Roles", recentTop5, false)}
        />
      </div>
  ...
  return (
    <div style={{ maxWidth:"680px" }}>
      ...
      <div style={{ background:T.card, border:`1px solid ${T.cardBdr}`, borderRadius:"14px", padding:"20px", marginBottom:"16px" }}>
        ...
        <Field label="Offer & Joining Tracker URL" fkey="offerTrackerUrl" testKey="offerTracker" placeholder="https://docs.google.com/spreadsheets/d/..." hint="Separate sheet: Name, Role, Centre, Offered released date, DOJ, Joining Status — powers Joining Ratio and Yet to Join cards" />
      </div>
      <div style={{ background:T.card, border:`1px solid ${T.cardBdr}`, borderRadius:"14px", padding:"20px", marginBottom:"16px" }}>
...
export default function App() {
  ...
  const openOfferModal = useCallback((title, entries) => {
    setModal({ title, showOffer: true, positions: entries.map((e, i) => ({
  ...
  const openDojModal = useCallback((title, entries) => {
    setModal({ title, showDoj: true, positions: entries.map((e, i) => ({
  ...
  const loadOfferData = useCallback(async (cfg) => {
    ...
    try {
      ...
      const entries = rows.filter(r => r["Name"] && r["Name"].trim()!=="").map(r => ({
        ...
        centre: (r["Centre"]||"").trim(), offerDate: (r["Offered released date"]||"").trim(),
        doj: (r["DOJ"]||"").trim(), joiningStatus: (r["Joining Status"]||"").trim(),
  ...
  const loadLiveData = useCallback(async (cfg) => {
    ...
    try {
      ...
      // parseSheetToPositions now returns a plain object { positions, avgTat, tatCount }
      const result = parseSheetToPositions(rows);
      if (result.positions.length > 0) {
        setData({ openPositions: result.positions, avgTat: result.avgTat, tatCount: result.tatCount });
        setIsLive(true);
        setToast({ message:`Loaded ${result.positions.length} positions · Avg TAT: ${result.avgTat != null ? result.avgTat+"d" : "N/A"} (${result.tatCount} closed roles)`, type:"success" });
      } else {
